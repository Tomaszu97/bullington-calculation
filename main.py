...
class Bullington:
    ...
    def plot_apexes(self):
        ...
        plt.plot(
            (self.TX[0], self.TX[0]),
            (self.TX[1], np.amin(self.terrain[:, 1])),
            c="#0066FF",
        )
        ...
        plt.plot(
            (self.RX[0], self.RX[0]),
            (self.RX[1], np.amin(self.terrain[:, 1])),
            c="#11CC11",
        )
        ...
        plt.plot(
            (self.O1[0], self.O1[0]),
            (self.O1[1], np.amin(self.terrain[:, 1])),
            c="#AA3333",
        )
        ...
        plt.plot(
            (self.O2[0], self.O2[0]),
            (self.O2[1], np.amin(self.terrain[:, 1])),
            c="#AA3333",
        )
        ...
        plt.plot(
            (self.BEQ[0], self.BEQ[0]),
            (self.BEQ[1], np.amin(self.terrain[:, 1])),
            c="#FF3300",
        )
        plt.text(
            self.BEQ[0], self.BEQ[1], " BEQ", c="#FF3300", fontsize=14, weight="bold"
        )

    def plot_lines(self):
        plt.plot(
            (self.TX[0], self.BEQ[0]),
            (self.TX[1], self.BEQ[1]),
            c="#FF9999",
            linestyle="dashed",
        )
        plt.plot(
            (self.RX[0], self.BEQ[0]),
            (self.RX[1], self.BEQ[1]),
            c="#FF9999",
            linestyle="dashed",
        )
        plt.plot(
    ...
    def calc_bullington_equivalent(self):
        ...
        if x > self.O1[0] and x < self.O2[0]:
            ...
            print("################################################################")
            print(
                "BŁĄD! Szczyt przeszkody zastępczej nie mieści się pomiędzy O1 i O2!\nSprawdż pozycje RX,TX,O1,O2!"
            )
            print("################################################################")
            exit()
    ...
    def place_object(self, position=0, rel_height=0, type="TX"):
        ...
        if type == "TX":
            ...
            raise Exception(
                f"{type}? Niepoprawna nazwa obiektu! Wybierz TX, RX, O1 lub O2"
            )

    def calc_geometry(self):
        a = math.sqrt(
            ((self.BEQ[0] - self.TX[0]) ** 2) + ((self.BEQ[1] - self.TX[1]) ** 2)
        )
        b = math.sqrt(
            ((self.BEQ[0] - self.RX[0]) ** 2) + ((self.BEQ[1] - self.RX[1]) ** 2)
        )
        d = math.sqrt(
            ((self.RX[0] - self.TX[0]) ** 2) + ((self.RX[1] - self.TX[1]) ** 2)
        )
    ...
    def calc_v(self):
        self.v = self.h * math.sqrt(
            2 * (self.d1 + self.d2) / (self.wavelength * self.d1 * self.d2)
        )

    def calc_fresnel_rn(self, n=1):
        return math.sqrt(
            (n * self.wavelength * self.d1 * self.d2) / (self.d1 + self.d2)
        )

    def calc_fresnel_r123(self):
        self.r1 = math.sqrt(
            (1 * self.wavelength * self.d1 * self.d2) / (self.d1 + self.d2)
        )
        self.r2 = math.sqrt(
            (2 * self.wavelength * self.d1 * self.d2) / (self.d1 + self.d2)
        )
        self.r3 = math.sqrt(
            (3 * self.wavelength * self.d1 * self.d2) / (self.d1 + self.d2)
        )
    ...
    def calc_C_S_F(self):
        self.C = (
            0.5
            - integrate.quad(
                lambda t: math.cos((math.pi / 2) * (t ** 2)), 0, self.v, limit=2000
            )[0]
        )
        self.S = (
            0.5
            - integrate.quad(
                lambda t: math.sin((math.pi / 2) * (t ** 2)), 0, self.v, limit=2000
            )[0]
        )
...
try:
    if filename == "kasina":
        ...
        bul.place_object(33300, 0, type="RX")
        bul.place_object(2100, 0, type="O1")
        bul.place_object(27850, 0, type="O2")
        ...
        bul.set_frequency(frequency)
        bul.place_object(0, 0, type="TX")
        bul.place_object(98098, 0, type="RX")
        bul.place_object(12477, 0, type="O1")
        bul.place_object(95100, 0, type="O2")
